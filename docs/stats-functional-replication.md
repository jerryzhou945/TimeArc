...
### §2.11 日视图分类环（StatsCategoryClock，取代三轨应用时钟）

- **规则**：12 小时表盘 + AM/PM 切换 + 60 刻度 + 1–12 时数字保留；表盘上只有
  **一条环**（中心 `0.64·base`、宽 `0.17·base`），一段弧 = 一段**同类别**的连续
  时间。环上不再有应用图标（身份下沉到悬停读数与应用库）。
- **为什么不是三轨**：旧 `StatsApplicationClock` 用三条同心轨道，只是为了躲开
  `foregroundSegmentsImpl` 的 60s 间隙合并造成的**跨 app 区间重叠**；半径不承载
  任何含义。单环必须把重叠**解掉**（扫描线，见下）而不是藏起来。
- **标准**：
  - **必须**：`StatsViewModel.buildCategoryRingRuns()` 先整天降噪，
    `projectCategoryRing()` 再按半天裁剪投影。**先降噪后裁剪**——反过来会把跨正午
    的长块截成碎片再被阈值误删。
  - **必须**：重叠归属取**更短**的 segment（长的那条是合并产生的假象，短的那条才是
    真实前台）；并列时按更晚开始、再按 groupKey，保证多次重算结果一致。
  - **必须**：两道门槛，回答两个不同问题——
    ① **噪声门槛** `minSeconds`（默认 60s，对齐后端 `kMergeGapSec`）：不是一段真实
    使用。前后都是空档→**丢弃**（唯一真正离开环的时间）。
    ② **可读门槛** `minArcDeg`（默认 2°，12h 盘上 1°=2min，即 4min）：看不见也点不中。
    只**并入**被它打断的块，孤立的则保留（是真实时间，由最小扫掠补到可见）。
    实测：只做①时真实一天出 47 段弧、其中 35 段不足 2°（条形码）；加②后 13 段、0 段不足。
  - **必须**：环只负责**形状**。中心总时长、图例秒数一律取**未过滤**的 `vmApps`
    聚合，绝不显示降噪后的数字，否则会与右侧 `DailyUsageShare` 自相矛盾。
  - **必须**：卡片脚注披露折叠量（`ringFoldedOnly` / `ringDroppedOnly` /
    `ringFoldedAndDropped` 三个模板，QML 选键，不拼字符串）。
  - **必须**：弧用 `lineCap="butt"`；平铺的弧用 round 端点会互相鼓起压盖。
  - **应当**：整天降噪结果缓存在 `_ringRuns`，AM/PM 切换只重投影（性能文档口径）。
  - **可选**：把 `minSeconds` 提升为设置页读层过滤项。
- **步骤**：①`buildCategoryRingRuns`（摊平→扫描线解重叠→按类别合并→两道门槛）；
  ②`projectCategoryRing`（裁半天→角度→最小扫掠）；③QML 单环 Canvas + 命中带 +
  图例/明细行 + 脚注。
