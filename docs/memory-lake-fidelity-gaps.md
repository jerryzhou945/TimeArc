...
## 月度回顾遮罩修复 + `RoundedFrame` 复用件（2026-06-03 晚）

月度回顾覆盖层的多屏内容踩了与卡面同一类坑：**`clip:true` 只裁矩形包围盒、不跟圆角**，故圆角容器里方角的
图片/填充棱角越过圆角边突出。把卡面那套遮罩配方抽成可复用的 **`RoundedFrame.qml`**（子项整体合成进
`layer.samples:4` 多重采样 layer → 单一圆角 `Rectangle` 作 `MultiEffect maskSource` → 顶层描边进同一被裁合成；
沿用 `maskThresholdMin:0.5 / maskSpreadAtMin:0.28`）。当容器用，子项直接写在内部，`radius` 设圆角
（= 半边长即正圆），`border.*` 设描边。已登记进 `qml/CMakeLists.txt`。

**逐处修复**
- **Step03/04/06**（海报 8幡出口 / split 千恋万花 / article P3R）：封面图原是 `Rectangle{radius;clip:true}`，
  四角方角突出 → 改走 `RoundedFrame`，图 + 渐变 + 文字一起裁到同一圆角。
- **Step05**（orbit ELDEN RING 环形头像）：`radius:110 + clip:true` 因 clip 是矩形，**整张渲染成方块**
  （圆角全失效）→ `RoundedFrame` 裁成真正的正圆。
- **Step02**（月历 7 柱填充条）：先只给填充条收**底角**（Qt6.7+ `bottomLeftRadius/bottomRightRadius`）治好底部；
  但**满条（Jun28 96%）顶边冲进容器顶部圆角区、方角漏顶**，且单独给填充条收顶角会因「填充顶弧心≠容器顶弧心」
  对不齐。正解：整格（底色+填充+文字）做成 `RoundedFrame`，填充保持方角由帧遮罩统一裁——矮条自然平顶水位线、
  满条顶部贴合容器圆角；0–100% 各档实测均干净。
- **外背板**：回顾不透明暗底原是方角，整屏读作「方角黑框套圆角壳」→ 暗底 `radius:24`（页面内嵌于 radius:32
  内容卡再内缩 8px，故 24 与内容卡圆角对齐），整屏读作一块圆角面板；圆角处只透出深色 App 背景，无页面内容穿帮。

**验证**：`qml.exe + grabToImage` 出图 + 真机走查（导航→开回顾→方向键逐屏）双确认；不透明图片帧另过品红 3×
逐像素门（正背及各帧 4 角 **poke=hole=0**）。⚠️ **品红逐像素门只适用于不透明内容**：Step02 填充条/底色是半透明
（填充 alpha 0.18–0.40），品红会透上来，逐像素门给假"漏"，这类只能靠肉眼渲染判定。`build.py` 干净、零新增 QML 告警。
