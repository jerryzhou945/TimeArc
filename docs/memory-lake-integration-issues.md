...
### [RESOLVED] B11 文案去通用词 + 封面淡化 + 站点取色错位（用户反馈三连）
- 日期：2026-06-03 ｜ 位置：§5 文案 / §4 视觉
- 用户反馈：(1) "穿插/平稳/日常"等词人人都会触发、不够 specialized、无分享欲；要"夜间使用"这类有代表性、
  独特、不尴尬的词。(2) Chrome 封面红色仍刺眼，封面也需淡化（约背景淡度的 80–90%，不必同样淡）。
  (2.5) bilibili 是 Chrome 里的网页，背景显示成了 Chrome 的色。
- 处置：
  1. **文案全面去通用词**：删 `patternLabel`；`moodWord` 改只用**有独特性的信号**——主要时段（清晨/上午/
     午间/午后/傍晚/夜间/深夜，新增"深夜"）+ 单次时长强度（沉浸/专注/追剧/畅聊/深读…），如"夜间聆听"
     "午后专注"；`analysisText` 改**纯事实**（时长/主要时段/单次最长/打开次数），去掉"系统据连续性识别为
     「穿插/平稳」"的尴尬断言；回顾 major/keywords/articleTitle/趋势"平稳"标题一并改为时段/强度词。
  2. **封面淡化**：新增 `coverTone`（比 `ambientTone` 略保留鲜明度、淡度约 80–90%），`GenerativeCover`
     渐变改用 coverTone——Chrome 红等被压成淡雅色，仍比背景略鲜明（焦点封面）。
  3. **站点取色错位**：USM 对 `site:*` 组（path 是宿主浏览器 exe）**不取图标色**，留空 -> QML 退回
     appColor 的站点品牌色（site:bilibili -> 粉）；通用规则避免任何站点组取到浏览器色。真机实测：QQ Music
     卡心情"夜间聆听"、分析纯事实；封面与背景均淡雅不刺眼。
