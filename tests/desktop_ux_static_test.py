...
def main():
    ...
    require(usage_h, "setAppIdentityOverrides", "application identity override API")
    require(usage_h, "validateCustomAppId", "application identity validation API")
    require(usage_cpp, 'item["originalGroupKey"]', "raw application identity in settings")
    require(usage_cpp, 'item["effectiveGroupKey"]', "effective application identity in settings")
    require(usage_cpp, "representativePathForGroup", "stable application icon path selection")
    require(settings_qml, '"app_identity_overrides"', "persisted application identity overrides")
    require(settings_qml, "saveAppIdentity", "inline application identity save action")
    require(settings_qml, "restoreAppIdentity", "application identity restore action")
    require(settings_qml, "pendingMergeKey", "explicit identity collision confirmation")
    require(settings_qml, "originalGroupKey", "read-only original application identity")
    require(i18n_js, "自定义应用 ID", "application identity editor translation")
