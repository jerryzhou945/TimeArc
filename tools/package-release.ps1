...
function Resolve-ExistingDirectory([string]$explicit, [string[]]$candidates, [string]$label) {
  if ($explicit) {
    if (-not (Test-Path $explicit -PathType Container)) {
      Write-Error "$label directory not found: $explicit"
      exit 1
    }
  ...
  Write-Error "$label directory not found; pass -$label explicitly"
  exit 1
}

$qtFromCommand = Get-Command windeployqt.exe -ErrorAction SilentlyContinue
$mingwFromCommand = Get-Command objdump.exe -ErrorAction SilentlyContinue
...
$qtVersion = if (Test-Path $qmake) { (& $qmake -query QT_VERSION).Trim() } else { "unknown" }
...
--- Qt 6 ($qtVersion): GNU LGPL-3.0 (with exceptions); some tools/add-ons GPL-3.0 ---
...
& (Join-Path $tools "verify-linkage.ps1") -Exe (Join-Path $stage "TimeArc.exe") `
  -DeploySupport $deploySupport -Objdump (Join-Path $MingwBin "objdump.exe")
