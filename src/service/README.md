...
API. The Qt app opens that database read-only for history, while
`usage_current.json` remains the live snapshot.
...
The normalized session mapping is described in `shared/usage_record.md`.
The JSON live snapshot is validated by `shared/usage_record.schema.json`.
